...
class CharacterLevelCNN(nn.Module):
    def __init__(self, config_path='../config.json',  n_classes=14, input_length=1014, input_dim=68,
        ...
        dimension = int((input_length - 96) / 27 * config['model_parameters']['conv'][-1][0])
        fc_layers = []
        for i, fc_layer_parameter in config['model_parameteres']['fc']:
            if i == 0:
                fc_layer = nn.Sequential(nn.Linear(dimension, fc_layer_parameter), nn.Dropout(0.5))
            elif i == len(config['model_parameteres']['fc']) - 1:
                fc_layer = nn.Sequential(nn.Linear(fc_layer_parameter, config['num_of_classes']))          
            else:
                fc_layer = nn.Sequential(nn.Linear(fc_layer_parameter, fc_layer_parameter), nn.Dropout(0.5))          
